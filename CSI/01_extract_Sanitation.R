...
variab <- 'sanitation_facilities'
files <- list.files(paste0(data_path,variab,'/'), full.names = T, recursive = T)
...
colnames(temp)[-1] <- gsub(".*_MEAN_|_Y.*","",names(temp)[-1])
df <- reshape2::melt(temp, variable.name = "date", value.name = paste0(variab),id.vars = "NAME_1")
knitr::kable(head(df, n=5))

filename <- paste0(root,variab,"/",iso,"/",iso,"_", variab,".csv")
