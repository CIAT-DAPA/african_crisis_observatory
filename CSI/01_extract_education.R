...
data_path <- '//alliancedfs.alliance.cgiar.org/WS18_Afrca_K_N_ACO/1.Data/Palmira/CSO/data/_global/'

iso <- 'SOM'

variab <- 'education'
gender <- 'difference'
files <- list.files(paste0(data_path,variab,'/'), full.names = T, recursive = T,pattern = gender)
img <- terra::rast(files)
...
colnames(temp)[-1] <- 2000:2017#gsub(".*_MEAN_|_Y.*","",names(temp)[-1])
df <- reshape2::melt(temp, variable.name = "date", value.name = paste0(variab, gender),id.vars = "NAME_1")
knitr::kable(head(df, n=5))

filename <- paste0(root,variab,"/",iso,"/",iso,"_", variab,gender,".csv")
