...
run_and_save_ecocrop <- function(grass_name, rain, tavg, aoi, ken_setl, uga_setl, ssd_setl) {
  ...
  grass_dir <- file.path("D:/OneDrive - CGIAR/SA_Team/Projects/AGNES/3_Data/3_Outputs/suitability_2021_2040", gsub(" ", "_", grass_name))
...
avg_suitability_path <- file.path("D:/OneDrive - CGIAR/SA_Team/Projects/AGNES/3_Data/3_Outputs/suitability_2021_2040/models", "average_suitability.tif")
writeRaster(avg_suitability, avg_suitability_path, overwrite = TRUE)

# Save the median suitability map
median_suitability_path <- file.path("D:/OneDrive - CGIAR/SA_Team/Projects/AGNES/3_Data/3_Outputs/suitability_2021_2040/models", "median_suitability.tif")
writeRaster(median_suitability, median_suitability_path, overwrite = TRUE)

# Plot the median suitability with additional details
median_suitability_jpg <- file.path("D:/OneDrive - CGIAR/SA_Team/Projects/AGNES/3_Data/3_Outputs/suitability_2021_2040/models", "median_suitability.jpg")
...
median_map <- tm_shape(median_suitability) +
  ...
  tm_layout(
    title = "Karamoja Pasture Suitability Index 2021-2040",
    title.size = 10.6,
    title.fontface = "bold",  
    title.position = c("center", "top"),
    legend.outside = TRUE,
