...
work_dir <- "D:/OneDrive - CGIAR/CIAT/PastureModelling_Turkana_Case/Coding/Turkana_County_Pasture_Modelling"
dir.create(work_dir, showWarnings = FALSE, recursive = TRUE)
setwd(work_dir)

# Define the path to the AOI shapefile
aoi_shapefile <- "D:/OneDrive - CGIAR/CIAT/PastureModelling_Turkana_Case/Coding/Turkana_County_Pasture_Modelling/igad_cluster_1_1/igad_cluster_1_1.shp"
...
rain_list <- lapply(iso_codes, function(iso) geodata::worldclim_country(iso, var="prec", path=work_dir))
tavg_list <- lapply(iso_codes, function(iso) geodata::worldclim_country(iso, var="tavg", path=work_dir))
...
ken_setl <- st_read('D:/OneDrive - CGIAR/SA_Team/korir/LULC/hotosm_ken_populated_places_points_shp.shp') %>%
  st_set_crs(st_crs(aoi)) %>%
  st_as_sf() %>%
  st_intersection(st_as_sf(aoi)) %>%
  filter(place == 'town')

uga_setl <- st_read('D:/OneDrive - CGIAR/SA_Team/korir/LULC/hotosm_uga_populated_places_points_shp.shp') %>%
  st_set_crs(st_crs(aoi)) %>%
  st_as_sf() %>%
  st_intersection(st_as_sf(aoi)) %>%
  filter(place == 'town')

ssd_setl <- st_read('D:/OneDrive - CGIAR/SA_Team/korir/LULC/SSD_PopulatedAreas_Dataset/ssd_pppls_ocha_20221216.shp') %>%
  st_set_crs(st_crs(aoi)) %>%
  st_as_sf() %>%
  st_intersection(st_as_sf(aoi))
...
run_and_save_ecocrop <- function(grass_name, rain, tavg, aoi, work_dir, ken_setl, uga_setl, ssd_setl) {
  ...
  grass_dir <- file.path(work_dir, "models", gsub(" ", "_", grass_name))
...
for (grass_name in grass_types) {
  tryCatch({
    run_and_save_ecocrop(grass_name, rain_aoi, tavg_aoi, aoi, work_dir, ken_setl, uga_setl, ssd_setl)
  }, error = function(e) {
