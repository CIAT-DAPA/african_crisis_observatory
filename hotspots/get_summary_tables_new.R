...
events_bp <- conf_data %>%
  ...
  group_by(EVENT_TYPE, short_label) %>%
...
write_csv(events_bp %>%
            ...
            mutate(freq = prop.table(n)), paste0(root, "_results/cluster_results/conflict/EVENTS_TYPE_barplot_df.csv"))
...
fata_bp <- conf_data %>%
  ...
  filter(!is.na(short_label)) %>%
  group_by(EVENT_TYPE, short_label) %>%
  summarise(counts = sum(FATALITIES), .groups = "drop") %>%
  na.omit()

write_csv(fata_bp %>% 
            dplyr::group_by(EVENT_TYPE) %>% 
            summarise(n = sum(counts), .groups = "drop") %>%
            mutate(freq = prop.table(n)),paste0(root, "_results/cluster_results/conflict/FATALITIES_barplot_df.csv"))
...
# lvls_clim <- clim_clust %>% 
#   dplyr::select(label, clust) %>% 
#   dplyr::filter(!duplicated(label)) %>% 
#   dplyr::arrange(desc(clust)) %>% 
#   pull(label) 
# 
# clim_clust$label <- factor(clim_clust$label, levels = lvls_clim)
# 
...
lapply(unique(clim_clust$label), function(i){
  
  cat("Making maps for: ", as.character(i) , "\n")
  
  # tmp_df <- conf_clim
  # tmp_df$label <-  as.character(tmp_df$label)
  # tmp_df$intersect_conf_clim[tmp_df$label != i] <- "Others combinations"
  # names(tmp_df)
  # lvls <- tmp_df %>% 
  #   dplyr::select(clim_cluster_short_label, clim_clust_order) %>% 
  ...
  #tmp_df$intersect_conf_clim <- factor(tmp_df$intersect_conf_clim, levels = lvls)
  ...
  #   pal_g <- brewer.pal(length(unique(tmp_df$intersect_conf_clim))-1 , main_col)
  # }else{
  #   pal_g <- rev(brewer.pal(length(unique(tmp_df$intersect_conf_clim))-1 , main_col))
  # }
...
for(i in get_ip_names){
  ...
  ht_rast <- terra::rast(paste0(root,"_results/hotspots/ip_maps/",iso, '_all_cat_hotspots_', ip_x, ".tif" )) %>% 
  terra::crop(., extent(shp_c)) %>% 
  terra::mask(., shp_c)
 ...
 # rast_labs %>%
   # dplyr::select(value_ID = valuID, label = final_label) %>% 
    #write_csv(., paste0(to_share_dir, "/", ip_x, "_hotspots_labels.csv"))
...
conf_clim <- conf_clim %>% 
  ...
  for(k in  na.omit(unique(rast_labs$category)) ){
    ...
    if(var_name == "NAME_0" | length(unique(shp_c[, var_name  ])) > 2){
      tmap_save(hots_map,
    ...
    lapply(unique(conf_clim$label), function(var){
      
      
      tmp_df <- conf_clim
      tmp_df$label <-  as.character(tmp_df$label)
      tmp_df$intersect_conf_clim[tmp_df$label != var] <- "Others combinations"
      
      lvls <- tmp_df %>% 
        dplyr::select(clim_cluster_short_label, clim_clust_order) %>% 
        dplyr::filter(!duplicated(clim_cluster_short_label)) %>% 
        dplyr::arrange(desc(clim_clust_order)) %>% 
        pull(clim_cluster_short_label) %>% 
        paste0(var,"-[", .,"]") %>% 
        c(., "Others combinations" )
      ...
      intersection <- st_intersects(tmp_df,shp_c)
      for  (i in 1:nrow(tmp_df)){
        tmp_df[i, 'GID_0'] <- lapply(intersection[i], function(x){shp_c$NAME_0[x]}) %>% unlist() %>% unique() %>% paste(., collapse = ",")
        
      }
      tmp_df$intersect_conf_clim <- droplevels(tmp_df$intersect_conf_clim)
      ...
      if(var == "Limited conflict"){
        ...
        pal_g <- rev(brewer.pal(length(unique(tmp_df$intersect_conf_clim)) , main_col))
      }
...
clusts_to_share <- st_as_sf(conf_clim)
...
eth_ext <- st_intersection(st_as_sf(clusts_to_share), st_as_sf(eth_c)) %>% 
  dplyr::group_by(ov) %>% 
  dplyr::summarize(eth_short_name = paste0(eth_short_name, collapse = ";"), eth_long_name = paste0(eth_long_name, collapse = ";") ) %>% 
  st_drop_geometry()
liveext <- st_intersection(st_as_sf(clusts_to_share), st_as_sf(livelihoods)) %>% 
  dplyr::group_by(ov) %>% 
...
clusts_to_share <- merge(x=clusts_to_share,y=liveext, by=c("ov"),all.x=TRUE, all=T)
clusts_to_share <- merge(x=clusts_to_share,y=eth_ext, by=c("ov"),all.x=TRUE, all=T)
...
row.names(clusts_to_share) <- sf::st_geometry(st_as_sf(clusts_to_share))$id

clusts_to_share <-st_as_sf(clusts_to_share)
clusts_to_share_gj <- sf_geojson(clusts_to_share)
geojsonio::geojson_write(clusts_to_share_gj, file = paste0(root, "/_results/clim_conflict_ips_overlays.geojson") )

as.data.frame(clusts_to_share) %>% writexl::write_xlsx(paste0(root, "_results/clim_conflict_ips_overlays.xlsx"))
