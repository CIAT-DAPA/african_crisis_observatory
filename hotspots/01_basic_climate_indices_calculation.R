...
#rgeos use R version 4.2.2
pkg <- c('sp', 'dplyr', 'readr', 'rgeos', 'tidyverse', 'geojsonsf',
         'readxl', 'geojsonlint', 'RColorBrewer', 'writexl', 'raster','terra', 'sf', 'stringr', 'stringi', 'rasterVis', 'maptools',
         'RColorBrewer','cowplot', 'grid','tmap', 'tmaptools', 'geojson', 'geojsonio', 'MetBrewer', 'paletteer', 'exactextractr',
         'lattice', 'latticeExtra')


installed_pkg <- pkg %in% rownames(installed.packages())
if (any(installed_pkg == FALSE)) {
  install.packages(pkg[!installed_pkg], dependencies = TRUE)
}
#' Packages loading
...
conf_clust_labs <- readr::read_csv(paste0(root, "_results/cluster_results/conflict/conflict_cluster_text_description.csv")) %>%
  dplyr::mutate(across(everything(.), as.character))
...
clusts_to_share@data <- clusts_to_share@data %>% 
  ...
  dplyr::select(  EVENTS,
                  ...
                  conflict_cluster_text_description = conflict_cluster_text_description,
