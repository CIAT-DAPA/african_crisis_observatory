  ...
  1:length(rstck) %>%
    purrr::map(., .f = function(i){
      ...
      rsaga.geoprocessor(lib    = 'statistics_regression',
                         ...
                         param  = list(PREDICTORS = paste0(root,'/srtm/',country,'/srtm_5km.tif'),
                                       REGRESSION = paste0(root,'/5km/',tolower(country),'/',gsub('.nc','',clnames[l]) %>% substr(.,1,4),'.',days[i],'.tif'),
                                       DEPENDENT  = paste0(root,'/50km/individuals/',gsub('.nc','',clnames[l]) %>% substr(.,1,4),'.',days[i],'.tif')),
                         env    = env)
